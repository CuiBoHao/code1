...
namespace CoorLib
{
    ...
    public class Gauss
    {
        ...
        private void EndPointLat(double[] Coeff, double x, ref double Bf1)
        {
            ...
            {
                Bf0 = Bf1;
                dX = Coeff[1] * Math.Sin(2 * Bf0) + Coeff[2] * Math.Sin(4 * Bf0)+ Coeff[3] * Math.Sin(6 * Bf0) + Coeff[4] * Math.Sin(8 * Bf0) + Coeff[5] * Math.Sin(10 * Bf0);
                Bf1 = (X - dX) / Coeff[0];
        ...
        private void Coefficient(ref double[] coef)
        {
            // double e1 = Ell.e1;
            double M0 = ell.M0;
            double A, B, C, D, E, F;
